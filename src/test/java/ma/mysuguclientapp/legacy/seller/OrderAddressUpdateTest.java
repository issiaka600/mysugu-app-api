...
/**
 * Contrat POST /api/v3/seller/orders/address-update du shim vendeur : écriture build-minimal de
 * {@code Commande.adresseLivraison} (embedded Localisation), scopée au restaurant du vendeur
 * authentifié. Une commande d'un AUTRE restaurant -> 404 (jamais de mutation cross-restaurant) —
 * garde de sécurité critique. Champs sans équivalent natif (zip, contact_person_name,
 * address_type) acceptés et ignorés (spec §6). Voir plan 3d.10.
 */
...
class OrderAddressUpdateTest {
    ...
    private final String owner1Email = "ord-addr1-" + System.nanoTime() + "@test.mysugu";
    private final String owner2Email = "ord-addr2-" + System.nanoTime() + "@test.mysugu";
    private final String clientEmail = "ord-addr-client-" + System.nanoTime() + "@test.mysugu";
    ...
    @Test
    void address_update_on_own_order_writes_localisation() throws Exception {
        String t1 = token(owner1Email);
        Map<String, Object> body = new HashMap<>();
        body.put("order_id", commande1Id);
        body.put("latitude", "34.02");
        body.put("longitude", "-6.83");
        body.put("address", "45 Avenue Test");
        body.put("contact_person_name", "Sara");
        body.put("phone", "0600000000");
        body.put("city", "Rabat");
        body.put("zip", "10000");
        body.put("email", "sara@test.mysugu");
        body.put("address_type", "home");

        Resp r = post("/api/v3/seller/orders/address-update", body, t1);
        assertThat(r.status).isEqualTo(200);
        assertThat(M.readTree(r.body).has("message")).isTrue();

        Commande updated = commandeRepo.findById(commande1Id).orElseThrow();
        assertThat(updated.getAdresseLivraison()).isNotNull();
        assertThat(updated.getAdresseLivraison().getLatitude()).isEqualTo(34.02);
        assertThat(updated.getAdresseLivraison().getLongitude()).isEqualTo(-6.83);
        assertThat(updated.getAdresseLivraison().getAdresse()).isEqualTo("45 Avenue Test");
    }

    @Test
    void address_update_on_other_restaurant_order_returns_404() throws Exception {
        String t1 = token(owner1Email);
        Map<String, Object> body = new HashMap<>();
        body.put("order_id", commande2Id);
        body.put("latitude", "34.02");
        body.put("longitude", "-6.83");
    ...
    private Commande newCommande(User client, Restaurant r, Plat p) {
        ...
        c.setNumeroCommande("ORD-ADDR-" + System.nanoTime());
        c.setClient(client);
        c.setRestaurant(r);
        ...
        c.setAdresseLivraison(new Localisation(1.0, 2.0, "ancienne adresse", "Casa", "20000", "Maroc"));
