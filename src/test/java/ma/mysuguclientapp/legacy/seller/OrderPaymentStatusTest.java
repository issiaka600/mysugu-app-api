...
/**
 * Contrat POST /api/v3/seller/orders/update-payment-status du shim vendeur : mise à jour du
 * statut de paiement (mapping 6valley -> natif StatutPaiement, spec §3b), scopée au restaurant du
 * vendeur authentifié. Une commande d'un AUTRE restaurant -> 404 (jamais de mutation
 * cross-restaurant) — garde de sécurité critique. Voir plan 3d.7.
 */
...
class OrderPaymentStatusTest {
    ...
    private final String owner1Email = "ord-pay1-" + System.nanoTime() + "@test.mysugu";
    private final String owner2Email = "ord-pay2-" + System.nanoTime() + "@test.mysugu";
    private final String clientEmail = "ord-pay-client-" + System.nanoTime() + "@test.mysugu";
    ...
    @Test
    void updates_own_order_payment_status_and_maps_to_native() throws Exception {
        String t1 = token(owner1Email);
        Resp r = post("/api/v3/seller/orders/update-payment-status",
                Map.of("order_id", commande1Id, "payment_status", "paid"), t1);
        assertThat(r.status).isEqualTo(200);
        assertThat(M.readTree(r.body).has("message")).isTrue();

        Commande updated = commandeRepo.findById(commande1Id).orElseThrow();
        assertThat(updated.getStatutPaiement()).isEqualTo(StatutPaiement.PAYE);
    }
    ...
    void other_restaurant_order_returns_404() throws Exception {
        String t1 = token(owner1Email);
        Resp r = post("/api/v3/seller/orders/update-payment-status",
                Map.of("order_id", commande2Id, "payment_status", "paid"), t1);
        assertThat(r.status).isEqualTo(404);
    }

    private Commande newCommande(User client, Restaurant r, Plat p) {
        Commande c = new Commande();
        c.setNumeroCommande("ORD-PAY-" + System.nanoTime());
        c.setClient(client);
        c.setRestaurant(r);
