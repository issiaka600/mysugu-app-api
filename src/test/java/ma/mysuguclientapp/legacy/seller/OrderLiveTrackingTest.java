...
/**
 * Contrat GET /api/v3/seller/orders/{id}/live-tracking du shim vendeur : adapte le tracking natif
 * (CommandeService.getCommandeTracking + TrackingLocationStore) vers la forme 6valley
 * {latitude, longitude, ...}, scopé au restaurant du vendeur authentifié. Jamais de 500 si aucune
 * position n'est connue. Voir plan 3d.9.
 */
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@TestPropertySource(properties = {"dispatch.auto.enabled=false"})
class OrderLiveTrackingTest {
    ...
    @Autowired TrackingLocationStore trackingLocationStore;

    private final String owner1Email = "ord-trk1-" + System.nanoTime() + "@test.mysugu";
    private final String owner2Email = "ord-trk2-" + System.nanoTime() + "@test.mysugu";
    private final String clientEmail = "ord-trk-client-" + System.nanoTime() + "@test.mysugu";
    private final String livreurEmail = "ord-trk-livreur-" + System.nanoTime() + "@test.mysugu";
    private Long owner1Id, owner2Id, clientId, livreurId, resto1Id, resto2Id, plat1Id, plat2Id;
    private Long commandeWithGpsId, commandeNoGpsId, commande2Id;
    ...
    void seed() {
        ...
        livreurId = newUser(livreurEmail, UserRole.LIVREUR).getId();
        ...
        commandeWithGpsId = newCommande(client, r1, p1).getId();
        commandeNoGpsId = newCommande(client, r1, p1).getId();
        commande2Id = newCommande(client, r2, p2).getId();

        trackingLocationStore.save(GpsLocationDTO.builder()
                .commandeId(commandeWithGpsId)
                .livreurId(livreurId)
                .latitude(33.5731)
                .longitude(-7.5898)
                .vitesse(20.0)
    ...
    @AfterEach
    void cleanup() {
        commandeRepo.findById(commandeWithGpsId).ifPresent(commandeRepo::delete);
        commandeRepo.findById(commandeNoGpsId).ifPresent(commandeRepo::delete);
    ...
    @Test
    void live_tracking_returns_known_position_for_own_order() throws Exception {
        String t1 = token(owner1Email);
        Resp r = get("/api/v3/seller/orders/" + commandeWithGpsId + "/live-tracking", t1);
        assertThat(r.status).isEqualTo(200);

        JsonNode n = M.readTree(r.body);
        assertThat(n.get("latitude").asDouble()).isEqualTo(33.5731);
        assertThat(n.get("longitude").asDouble()).isEqualTo(-7.5898);
    }

    @Test
    void live_tracking_returns_benign_shape_when_no_position_known() throws Exception {
        String t1 = token(owner1Email);
        Resp r = get("/api/v3/seller/orders/" + commandeNoGpsId + "/live-tracking", t1);
        assertThat(r.status).isEqualTo(200);

        JsonNode n = M.readTree(r.body);
        assertThat(n.has("latitude")).isTrue();
        assertThat(n.get("latitude").isNull()).isTrue();
    }

    @Test
    void live_tracking_of_other_restaurant_order_returns_404() throws Exception {
        String t1 = token(owner1Email);
        Resp r = get("/api/v3/seller/orders/" + commande2Id + "/live-tracking", t1);
        assertThat(r.status).isEqualTo(404);
    }

    private Commande newCommande(User client, Restaurant r, Plat p) {
        Commande c = new Commande();
        c.setNumeroCommande("ORD-TRK-" + System.nanoTime() + "-" + Math.random());
        c.setClient(client);
        c.setRestaurant(r);
